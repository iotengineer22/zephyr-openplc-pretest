...
// Suppress warnings for IEC library code
...
static inline TIME __time_mul(TIME IN1, LREAL IN2){
  ...
  TIME res = {(int32_t)((long)s + ns.quot),
		      (int32_t)((long)ns.rem + (s_f - s) * 1000000000) };
  __normalize_timespec(&res);
  return res;
}
static inline TIME __time_div(TIME IN1, LREAL IN2){
  ...
  time_t s = (time_t)s_f;
  TIME res = {(int32_t)s,
              (int32_t)((long)(IN1.tv_nsec / IN2 + (s_f - s) * 1000000000)) };
  __normalize_timespec(&res);
  return res;
}
...
static inline LREAL __string_to_real(STRING IN) {
    ...
    /*   TO_TIME   */
    /***************/
static inline TIME    __int_to_time(LINT IN)  {return (TIME){(int32_t)IN, 0};}
static inline TIME   __real_to_time(LREAL IN) {return (TIME){(int32_t)IN, (int32_t)((IN - (LINT)IN) * 1000000000)};}
static inline TIME __string_to_time(STRING IN){
    ...
    if(l != 0){
        ...
        return  (TIME){(int32_t)IN_val, (int32_t)((long)(IN_val - (LINT)IN_val)*1000000000)};
    }else{
        return  (TIME){(int32_t)__pstring_to_sint(&IN), 0};
    }
}
